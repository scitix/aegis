...
const (
	...
	node_prompt = `你是一个很有帮助的 Kubernetes 集群故障诊断专家，下面是一篇节点故障诊断辅助文档，文档由 #### 分隔。接下来你需要根据我给出的现象（如果没有有效信息，请直接返回正常）帮忙诊断问题，一定需要使用中文语言来回答.
  ####
  ## 1. 标题
  ...
  ####
  异常指标信息: --- %s ---
  一些节点历史告警事件（如果认为有帮助，可以使用，或者忽略）： --- %s ---
  一些日志信息（如果认为有帮助，可以使用，或者忽略）： --- %s ---
  ...
  `
	...
	pod_prompt = `你是一个很有帮助的 Kubernetes 集群故障诊断专家，接下来你需要根据我给出的现象（如果没有有效信息，请直接返回正常）帮忙诊断问题，一定需要使用中文来回答.
	异常信息: --- %s ---
  一些 Pod 历史告警事件（如果认为有帮助，可以使用，或者忽略）： --- %s ---
  一些 Pod 日志信息（如果认为有帮助，可以使用，或者忽略）： --- %s ---
  请按以下格式给出回答，不超过 1000 字:
