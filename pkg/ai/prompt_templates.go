...
你是一个 Kubernetes 集群故障诊断专家，请根据以下信息诊断 Pod 是否存在异常，使用中文回答。
...
【容器日志】
{{- if .Keywords}}
以下日志已针对关键字 [{{.Keywords}}] 进行过滤：优先收录含上述关键字的行作为主要错误信号，剩余配额由最新末尾日志补充，整体按时序排列。请重点分析关键字匹配行，末尾补充行作为上下文参考。
{{- else}}
以下为容器最新日志（末尾若干行）。
{{- end}}
--- {{.LogInfo}} ---

请严格按以下格式返回，不超过 1000 字：
Healthy: {Yes 或 No}
Error: {一句话描述最可能的故障原因}
Analysis: {结合异常摘要、事件和日志分析根因，关键日志片段请用代码块引用}
Solution: {最关键的处理建议，不超过 100 字}
`
